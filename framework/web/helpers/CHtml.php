	...
	protected static function clientChange($event,&$htmlOptions)
	{
		...
		if(isset($htmlOptions['submit']))
		{
			...
			if($htmlOptions['submit']!=='')
				...
				$url='';
			$handler.="jQuery.yii.submitForm($id,'$url',$params);{$return};";
		}
		...
		{
			//$confirm='alert(\''.CJavaScript::quote($htmlOptions['confirm']).'\')';	
			$confirm_msg = Yii::t('strings','Are you sure you want to delete this item?');		
			if($handler!=='')
				$handler="bootbox.confirm('".$confirm_msg."',function(result){if(result){".$handler."}})";
			else
